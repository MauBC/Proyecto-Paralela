...
if rank == 0:
    ...
    print("\n===== RESULTADOS DEL KNN PARALELO =====")
    print(f"Procesos utilizados (p): {size}")
    print(f"Accuracy: {accuracy:.4f}")
    print(f"Tiempo Total: {t_total_end - t_total_start:.4f} s")
    print(f"Tiempo Comunicación (Broadcast + Scatter + Gather): "
          f"{(t_bcast_end - t_bcast_start) + (t_scatter_end - t_scatter_start) + (t_gather_end - t_gather_start):.4f} s")
    print(f"Tiempo Computación (KNN puro): {t_compute_end - t_compute_start:.4f} s")

    # Guardar CSV para gráficos
    with open("knn_times.csv", "a") as f:
        f.write(f"{size},{accuracy:.4f},{t_total_end - t_total_start:.4f},"
                f"{t_compute_end - t_compute_start:.4f},"
                f"{(t_bcast_end - t_bcast_start) + (t_scatter_end - t_scatter_start) + (t_gather_end - t_gather_start):.4f}\n")
